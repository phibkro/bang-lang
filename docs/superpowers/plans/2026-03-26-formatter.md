...
const formatExpr = (expr: Ast.Expr): Doc.Doc<never> =>
  Match.value(expr).pipe(
    ...
    Match.tag("DotAccess", (e) =>
      Doc.cat(formatExpr(e.object), Doc.cat(Doc.text("."), Doc.text(e.field))),
    ),
...
it.effect("formats block expression", () =>
  Effect.gen(function* () {
    expect(yield* fmt("result = { x = 1; y = 2; x + y }")).toBe(
      "result = { x = 1; y = 2; x + y }\n",
    );
  }),
);
...
Match.tag("Block", (e) => {
  if (e.statements.length === 0) {
    ...
    return Doc.group(
      Doc.cat(
        Doc.text("{"),
        Doc.catWithSpace(Doc.empty, Doc.catWithSpace(formatExpr(e.expr), Doc.text("}"))),
      ),
    );
  }
